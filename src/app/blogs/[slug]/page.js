                     ...
                     <div className='py-3  '>
                        {data?.content?.map((item, index) => (
                           <div key={index} className='space-y-4'>
                              {item.heading && <h1 className={`${item.size?'text-[1.6rem] md:text-[1.9rem]':'text-[2rem] md:text-[2.5rem] '}`}>{item.heading}</h1>}
                              {item.desc.length > 0 && item?.desc.map((subitem, descIndex) => (
                                 <p className='pb-4' key={descIndex} dangerouslySetInnerHTML={{__html:subitem}}></p>
                              ))}
